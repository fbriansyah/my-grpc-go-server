...
import (
	...
	dresl "github.com/fbriansyah/my-grpc-go-server/internal/application/domain/resiliency"
	...
	for {
		...
		if err == io.EOF {
			res := resl.ResiliencyResponse{
				DummyString: fmt.Sprintf("Receive %v requests from client", strconv.Itoa(i)),
			}
...
func (a *GrpcAdapter) BiDirectionalResiliency(stream resl.ResiliencyService_BiDirectionalResiliencyServer) error {
	...
	for {
		...
		default:
			...
			str, sts := a.resiliencyService.GenerateResiliency(
				req.MinDelaySecond,
				req.MaxDelaySecond,
				req.StatusCodes,
			)
			...
			err = stream.Send(&resl.ResiliencyResponse{
				DummyString: str,
			})
